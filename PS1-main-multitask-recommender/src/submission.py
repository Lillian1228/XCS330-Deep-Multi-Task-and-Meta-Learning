...
class MultiTaskNet(nn.Module):
    ...
    def forward_with_embedding_sharing(self, user_ids, item_ids):
        ...
        ### START CODE HERE ###
        predictions = ((self.U(user_ids) * self.Q(item_ids)).sum(dim=1) + self.A(user_ids).squeeze() + self.B(item_ids).squeeze()).squeeze()    
    ...
    def forward_without_embedding_sharing(self, user_ids, item_ids):
        ...
        predictions = score = None
        ### START CODE HERE ###
        predictions = ((self.U_reg(user_ids) * self.Q_reg(item_ids)).sum(dim=1) + self.A(user_ids).squeeze() + self.B(item_ids).squeeze()).squeeze()    
