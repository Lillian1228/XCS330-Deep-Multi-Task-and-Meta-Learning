...
class ProtoNet:
    ...
    def _step(self, task_batch):
        ...
        for task in task_batch:
            ...
            ### Find the prototypes for each class in the support set
            
            # number of classes in a task
            num_ways = (torch.max(labels_support) + 1).item()

            # to prevent gradient calculation
            with torch.no_grad():
                # y_s mapping from support images to features
                y_s = self._network(images_support) #.detach()  # (ways * support, features)
            ...
            # y_q mapping from query images to features
            y_q = self._network(images_query)  # (num_query, features)
        
            ### Compute distance between query examples and each of the prototypes - squared euclidean distance
            ...
            # Compute negative squared distances as logits
            # Get cross-entropy loss between the logits and labels_query
